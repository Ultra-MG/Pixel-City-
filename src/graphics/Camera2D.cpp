...
void Camera2D::move(sf::Vector2f deltaWorld) {
...
void Camera2D::setCenter(sf::Vector2f worldCenter) {
  m_view.setCenter(worldCenter);
}

sf::Vector2f Camera2D::screenToWorld(sf::Vector2f screenInternal) const {
  const sf::Vector2f viewSize = m_view.getSize();
  const sf::Vector2f topLeft  = m_view.getCenter() - viewSize * 0.5f;

  // Convert "internal screen pixels" to world units based on current view size
  const float sx = viewSize.x / m_baseSize.x;
  const float sy = viewSize.y / m_baseSize.y;

  return topLeft + sf::Vector2f(screenInternal.x * sx, screenInternal.y * sy);
}
